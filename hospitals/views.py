...
# eta doctor page er sathe conect 
...
def login_from(request):
    ...
    pwd = ''
    if request.method=="POST":
        m=sql.connect(host="localhost",user="root",password="root",database='hospital')
        cursor=m.cursor()
        d=request.POST
        ...
        c="select * from registation where email='{}' and password='{}'".format(em,pwd)
        cursor.execute(c)
...
def register_form(request):
    ...
    if request.method == 'POST':
        ...
        c="insert into registation Values('{}','{}','{}','{}','{}')".format(fn,ln,s,em,pwd)
        cursor.execute(c)
        m.commit()
...
def testbooking(request):
    return render(request, 'testbooking.html')
